...
class MainWindow():
    def __init__(self):
        ...
        Label(self.win,text="Cisco IOS:", relief='groove',anchor="w").place(x=30,y=10,width=120,height=30)
        Label(self.win, text="Choose folder to save SN(Optional)：", relief='groove', anchor="w").place(x=30, y=120, width=200, height=30)
        Label(self.win, text="Network devices' IP：", relief='groove', anchor="w").place(x=250, y=10, width=180, height=30)
        Label(self.win, text="Found SN:", relief='groove', anchor="w").place(x=500, y=10, width=250, height=30)
        ...
        Button(self.win, text="点击生成序列号",command=self.getdomainuser).place(x=500, y=520, width=120, height=40)
    ...
    def last(self,*event):
        if self.entryuser.get()!='' and self.entrypwd.get()!='':
            ...
            commands = ['do show version','exit']
            secret = self.entryenable.get()
            ios = self.val.get()
            time_start=time.strftime('%X')
            if self.dic!='':
                f1 = open(self.dic + "\\" + date + "_failed_SN.txt", 'a+')
                f = open(self.dic + "\\" + date + "_SN.txt", 'a+')
                for ip in self.ips:
                    t = threading.Thread(target=getSN.sn_ssh,
                                         args=(username, password, ip, commands, secret, f, f1, Queue(), ios))
                    t.start()
                    threads.append(t)
                for t in threads:
                    t.join()
                ...
                for ip in self.ips:
                    t = threading.Thread(target=getSN1.sn_ssh,
                                         args=(username, password, ip, commands, secret, Queue(), ios))
                    t.start()
                    threads.append(t)
                for t in threads:
                    t.join()
                if len(generalVar.failed_sn)>0:
                    showerror('未成功获取到SN的设备：',generalVar.failed_sn)
                    generalVar.failed_sn=[]

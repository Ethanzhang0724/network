...
class MainWindow():
    def __init__(self):
        ...
        Label(self.win, text="The IP You want to Search", relief='groove', anchor="w").place(x=250, y=10, width=180, height=30)
        Label(self.win, text="The Mac You want to Search", relief='groove', anchor="w").place(x=250, y=290, width=180,height=30)
        ...
        self.site_items=("YZ Campus",'YZ Prd',"SZ Campus","SZ Prd","Shanghai Office",'JZ Campus','Subang','SG Office','JP Office','AU Office','KR office')
        self.site_ips=("10.130.192.2","10.130.211.2",'10.130.213.2','10.130.203.2','10.130.148.2','10.130.204.2','10.130.220.2','10.130.209.2','10.130.210.2',
                       '10.130.170.3','10.130.212.67')
    ...
    def getdomainuser(self,*event):
        str_ip = self.text1.get(0.0, END).strip()
        str_mac = self.text3.get(0.0, END).strip()
        try:
            if len(str_ip)>0:
                self.mac=str_ip
                ...
                self.root.mainloop()
            elif len(str_mac)>0 and len(str_ip)==0:
               self.mac = EUI(str_mac)
               self.mac.dialect = mac_cisco
               self.index = self.site_items.index(self.val2.get())
               self.ip = self.site_ips[self.index]
               print(self.ip)
               self.root = Toplevel()
    ...
    def last(self,*event):
        if self.entryuser.get()!='' and self.entrypwd.get()!='':
            ...
            self.text2.insert(INSERT, generalVar.mac_text + '\n')      #mac地址信息
            self.text3.delete(0.0, END)
            self.text3.insert(INSERT, generalVar.mac_text1 + '\n')
                                   #mac不为空,则清空ip文本框
        else:

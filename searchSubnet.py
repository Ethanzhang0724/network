...
class MainWindow():
    def __init__(self):
        ...
        Label(self.win, text="网段信息", relief='groove', anchor="w").place(x=290, y=10, width=250, height=30)
    ...
    def getlast(self,*event):
        ...
        if len(self.ips)>0 :
            ...
            for i in self.ips:
                ...
                self.text2.insert(INSERT, "网络地址: "+str(subnet1.network)+'\n')
                self.text2.insert(INSERT, "子网掩码： " + str(subnet1.netmask)+'\n')
                self.text2.insert(INSERT, "广播地址： " + str(subnet1.broadcast)+'\n')
                self.text2.insert(INSERT, "反掩码（掩码通配符: "+str(subnet1.hostmask)+'\n')
                self.text2.insert(INSERT, "可用IP数量：" + str(int(subnet1.size)-2) + '\n\n')

...
class MainWindow():
    def __init__(self):
        ...
        Label(self.win, text="选择或创建文件夹以保存配置文件：", relief='groove', anchor="w").place(x=30, y=120, width=200, height=30)
        Label(self.win, text="输入网络设备IP地址：", relief='groove', anchor="w").place(x=250, y=10, width=180, height=30)
        Label(self.win, text="保存配置的命令：", relief='groove', anchor="w").place(x=500, y=10, width=250, height=30)
        ...
        self.text2.insert(INSERT, '(将要执行的命令如下(可修改):)\n')
        commands = ['do sh ip ospf neighbor', 'do sh interfaces status', 'do sh ip inter brief', 'do sh cdp neighbors',
                    'do sh ip route',
                    'do sh vlan', 'do show vtp status', 'do sh run','end']
        for i in commands:
    ...
    def getdomainuser(self):
        ...
        if len(self.ips)>0 and len(self.commands)>0 and self.dic!='':
            ...
            self.root.title('信息验证通过,请输入登录设备的账号')

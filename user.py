...
class MainWindow():
    def __init__(self):
        ...
        x = (self.MainWindow.winfo_screenwidth() - 307) / 2
        y = (self.MainWindow.winfo_screenheight() - 267) / 2
        self.MainWindow.geometry('307x267+%d+%d' % (x, y))
        ...
        self.enuserPwd = Entry(self.MainWindow,show='*')  # 用户密码
    ...
    # 查询用户信息，并显示在表格中
    def query(self):
        result = service.query("select username from tb_user")  # 调用服务类中的公共方法执行查询语句
    ...
    def edit(self):
        try:
            if self.tree.focus() != "":  # 判断是否选择了要修改的数据
                ...
                userName=self.enuserId.get()
                if userName==service.username or service.username=='zhang':
                    if userPwd != "":  # 判断密码不为空
                        # 执行修改操作
                        result = service.exec("update tb_user set userPwd= %s where userName=%s",
                                              (userPwd, self.enuserId.get()))
                        if result > 0:  # 如果结果大于0，说明修改成功
                            self.query()  # 在表格中显示最新数据
                            showinfo('提示', '信息修改成功！')
                else:
                    showwarning('警告', '您没有权限执行编辑操作！')
        except:
    ...
    def delete(self):
        try:
            if self.tree.focus() != "":  # 判断是否选择了要删除的数据
                userName = self.tree.set(self.tree.focus())["userId"]
                if userName == service.username or service.username == 'zhang':
